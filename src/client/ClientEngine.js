...
class ClientEngine {
  ...
  loadSprites(spritesGroup) {
    ...
    const spritesGroupKeys = Object.keys(spritesGroup);

    spritesGroupKeys.forEach((groupName) => {
      const group = spritesGroup[groupName];
      this.sprites[groupName] = group;

      const groupKeys = Object.keys(group);

      groupKeys.forEach((spriteName) => {
  ...
  renderSpriteFrame(cfg) {
    // eslint-disable-next-line
    const { sprite, frame, x, y, w, h } = cfg;
